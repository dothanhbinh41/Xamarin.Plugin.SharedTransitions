    ...
    public class SharedTransitionShell : Shell, ISharedTransitionContainer
    {
        ...
        private void ShellSectionOnChildAdded(object sender, ElementEventArgs e)
        {
            if (e.Element is ITransitionAware aware)
            {
                ...
                MessagingCenter.Subscribe<SharedTransitionShell, SharedTransitionEventArgs> (e.Element, "SendTransitionStarted", (senderx, args) =>
                {
                    if (page == args.PageFrom || page == args.PageTo)
                        aware.OnTransitionStarted(args);
                });

                MessagingCenter.Subscribe<SharedTransitionShell, SharedTransitionEventArgs> (e.Element, "SendTransitionEnded", (senderx, args) =>
                {
                    if (page == args.PageFrom || page == args.PageTo)
                        aware.OnTransitionEnded(args);
                });

                MessagingCenter.Subscribe<SharedTransitionShell, SharedTransitionEventArgs> (e.Element, "SendTransitionCancelled", (senderx, args) =>
                {
                    if (page == args.PageFrom || page == args.PageTo)
